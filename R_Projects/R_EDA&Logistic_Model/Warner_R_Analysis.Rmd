...
```{r Bar Chart by HDSize and Average Price}
...
```{r Subsetting data for modeling}
set.seed(1)
...
```{r Creating Tayko Linear Model}
lm_model<-lm(Spending~Freq+last_update_days_ago+Web.order+Address_is_res+US, data=tayko_data)
summary(lm_model)
```

```{r}
#The purchaser is most likely to spend a large amount of money if it is someone who purchases frequently, has a low last update time, orders on the web, and address is not a residency.
...
```{r Creating New Metrics to Understand Data}
bad_buys<- cars%>%
...
```{r Looking at car price by year}
...
```{r Frequency of bad cars over time by make}
...
```{r Plotting performance of top 3 American cars by Auction Price}
...
```{r Logistic model to predict whether or not the car will be considered a bad buy}
